  ...
  return (
    <div className="min-h-screen pt-24 pb-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 font-sans">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-950/70 border border-purple-800 text-purple-300 text-xs font-sans font-semibold uppercase tracking-wider">
            <Lock className="w-3.5 h-3.5 text-purple-400" /> Elevated Security Operations Center
          </div>
          <h1 className="text-3xl font-extrabold text-white mt-1 font-sans tracking-tight">Admin Command & Telemetry</h1>
        ...
        <button
          ...
          disabled={loading}
          className="btn-cyber-secondary text-xs py-2 px-4 flex items-center gap-1.5 font-sans font-medium"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 font-sans">
        <div className="glass-card p-5 rounded-2xl space-y-2">
          ...
          <div className="text-[11px] text-slate-400 font-sans">
            <span className="font-mono">{telemetry?.telemetry?.spoof_detected_24h || 0}</span> spoof / <span className="font-mono">{telemetry?.telemetry?.bona_fide_24h || 0}</span> authentic
          </div>
        </div>
        ...
        <div className="glass-card p-5 rounded-2xl space-y-2">
          ...
          <div className="text-[11px] text-red-400/80 font-sans">Forensic threat index</div>
        </div>
        ...
        <div className="glass-card p-5 rounded-2xl space-y-2">
          ...
          <div className="text-[11px] text-amber-400/80 font-sans">BullMQ Async Workers</div>
        </div>
        ...
        <div className="glass-card p-5 rounded-2xl space-y-2">
          ...
          <div className="text-2xl font-bold font-sans text-emerald-400 uppercase">
            {telemetry?.telemetry?.ml_service_status || 'healthy'}
          </div>
          <div className="text-[11px] text-emerald-400 font-sans">FastAPI + PyTorch GPU/CPU</div>
        </div>
      </div>

      {/* Audit Log Stream */}
      <div className="glass-card p-6 rounded-2xl space-y-4">
        <h3 className="text-sm font-bold text-white flex items-center gap-2 font-sans tracking-tight">
