    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-16 cyber-grid-bg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 font-sans">
        {FEATURES.map((f, idx) => {
          const Icon = f.icon;
          return (
            <div key={idx} className="glass-card p-6 space-y-4 glass-card-hover font-sans">
              <div className="flex items-center justify-between">
                <div className="p-3 rounded-xl bg-gray-50 border border-gray-200 text-gray-900">
                  <Icon className="w-5 h-5" />
                </div>
                <span className="text-[10px] font-sans px-2.5 py-0.5 rounded-full bg-gray-50 border border-gray-200 text-gray-900 font-semibold">
                  {f.tag}
                </span>
              </div>

              <div className="space-y-1">
                <h3 className="text-base font-bold text-gray-900 font-sans">{f.title}</h3>
                <p className="text-xs text-gray-600 leading-relaxed font-sans">{f.desc}</p>
              </div>

              <div className="pt-2 border-t border-gray-200 space-y-1.5 font-sans">
                {f.benefits.map((b, i) => (
                  <div key={i} className="flex items-center gap-2 text-[11px] text-gray-600 font-sans">
      ...
      <div className="glass-panel p-8 rounded-3xl border border-gray-200 text-center space-y-4 bg-gradient-to-r from-[#3B82F6]/10 via-[#0B1628] to-[#3B82F6]/10 font-sans">
        <h3 className="text-xl font-bold text-gray-900 font-sans">Ready to inspect voice threats in real time?</h3>
        <Link
          to="/detect"
