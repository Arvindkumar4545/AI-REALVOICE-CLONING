    ...
    <div className="glass-panel p-6 rounded-2xl border border-gray-200 bg-white space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-200 pb-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-500 font-sans">Combined Threat Index:</span>
          <span
          ...
          >
            <span className="font-mono">{copilotData?.fraud_risk_score}</span> / 100 ({copilotData?.risk_tier})
          </span>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="text-[11px] text-gray-500">Auto-correlated progression</span>
        </div>
          ...
          <div className="flex items-center gap-2 min-w-max">
            {copilotData?.storyline_nodes.map((node, i) => (
              <React.Fragment key={node.id}>
                <div
                  className={`px-3 py-2 rounded-xl text-xs font-sans font-medium flex items-center gap-1.5 shadow-sm ${
                    node.status === 'danger'
      ...
      {copilotData?.active_warning && (
        <div className="p-4 rounded-xl bg-red-50/80 border border-red-200 space-y-2">
          <div className="flex items-center gap-2 text-red-700 font-semibold text-xs uppercase tracking-wider font-sans">
            <AlertTriangle className="w-4 h-4" />
            <span>Active Defense Recommendation</span>
          </div>
          <ul className="space-y-1 text-xs text-red-900 font-sans">
      ...
      <div className="space-y-2">
        <h4 className="text-xs font-semibold uppercase tracking-wider text-gray-700 font-sans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {copilotData?.detected_intents.map((item) => (
            <div
              key={item.intent}
              className="p-3 rounded-xl bg-white border border-gray-200 text-xs font-sans space-y-1.5 shadow-sm"
            >
              <div className="flex items-center justify-between font-semibold">
                <span className="text-gray-900">{item.label}</span>
                <span className="text-red-600 font-mono font-bold">+{item.risk_contribution}</span>
              </div>
              ...
              <div className="pt-1 flex flex-wrap gap-1">
                {item.matched_phrases.map((phrase, pi) => (
                  <span
                    key={pi}
                    className="px-2 py-0.5 rounded bg-gray-100 text-[11px] text-gray-700 font-sans italic"
                  >
                    "{phrase}"
                  </span>
                ))}
