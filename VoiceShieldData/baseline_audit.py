...
if train_dev_speaker_leak:
    print(f"   WARNING: {len(train_dev_speaker_leak)} speakers appear in both train and dev")
print()
...
# 6. Check corrupted/unreadable files
print("6. FILE INTEGRITY CHECK")
print("-" * 80)
import soundfile as sf

corrupted = {"train": [], "dev": [], "eval": []}
missing = {"train": [], "dev": [], "eval": []}

for split_name, split_data in [("train", train_rows), ("dev", dev_rows), ("eval", eval_rows)]:
    for idx, row in split_data.iterrows():
        path = Path(row["path"])
        if not path.exists():
            missing[split_name].append(row["path"])
        else:
            try:
                sf.info(str(path))
            except Exception as e:
                corrupted[split_name].append((row["path"], str(e)[:50]))

print(f"   Train missing: {len(missing['train'])}, corrupted: {len(corrupted['train'])}")
print(f"   Dev missing: {len(missing['dev'])}, corrupted: {len(corrupted['dev'])}")
print(f"   Eval missing: {len(missing['eval'])}, corrupted: {len(corrupted['eval'])}")

if any(corrupted.values()) or any(missing.values()):
    print()
    print("   Details:")
    for split_name in ["train", "dev", "eval"]:
        if corrupted[split_name]:
            print(f"   {split_name.upper()} corrupted files (showing first 3):")
            for path, err in corrupted[split_name][:3]:
                print(f"      {Path(path).name}: {err}")
        if missing[split_name]:
            print(f"   {split_name.upper()} missing files (showing first 3):")
            for path in missing[split_name][:3]:
                print(f"      {path}")
print()
...
if len(duplicates) > 0:
    ...
    print(f"   Examples:")
    for path in duplicates["path"].unique()[:3]:
        print(f"      {path}")
print()
...
print("-" * 80)
usable_train = len(train_rows) - len(corrupted["train"]) - len(missing["train"])
usable_dev = len(dev_rows) - len(corrupted["dev"]) - len(missing["dev"])
usable_eval = len(eval_rows) - len(corrupted["eval"]) - len(missing["eval"])

print(f"   Train: {usable_train:7d} (total: {len(train_rows)}, corrupted: {len(corrupted['train'])}, missing: {len(missing['train'])})")
print(f"   Dev:   {usable_dev:7d} (total: {len(dev_rows)}, corrupted: {len(corrupted['dev'])}, missing: {len(missing['dev'])})")
print(f"   Eval:  {usable_eval:7d} (total: {len(eval_rows)}, corrupted: {len(corrupted['eval'])}, missing: {len(missing['eval'])})")
print()
